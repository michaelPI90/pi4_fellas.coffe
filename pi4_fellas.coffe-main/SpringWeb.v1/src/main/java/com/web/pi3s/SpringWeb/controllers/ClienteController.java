...
public class ClienteController {
  ...
  @RequestMapping("/clienteLogin")
  public String login(Model model, Clientemodels cliente) {
  
        return "/loginCliente/loginCliente";

  }

  @PostMapping("/logar")
  String listarUsuario(Model model, Clientemodels cliente) {
      Optional<Clientemodels> pesquisarCliente = this.repository.findByEmail(cliente.getEmail());

      String erroMsg = null;
      if (pesquisarCliente.isEmpty()) {
          erroMsg = "Email não encontrado!!!";
      } else if (!enconder.matches(pesquisarCliente.get().getSenha(), pesquisarCliente.get().getSenha())) {
